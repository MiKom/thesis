...
Another class of devices that OpenCL cat be used on, are GPUs. In fact, CUDA and
later OpenCL emerged from attempts to tame computing potential of these
highly--parallel graphical unis.

Implementation on one such device, GeForce GTX580 from NVIDIA corporation, based
on architecture called \emph{Fermi} will be presented based on \cite[p59]{gaster2012heterogeneous}
and \cite{nvidiafermi}.

GTX 580 is the most advanced graphic card in GeForce series 500 lineup. It
consists of 16 Compute Units called \emph{Symmetric Multiprocessors} (SMs).

Each SM contains the following elements (see \autoref{fig:fermism}):

\begin{description}
  \item[32 \emph{CUDA Cores}] compute units containing ALU\footnote{Arithmetic Logic Unit}
  ...
  \item[16 load/store units] for calculation of addresses for memory access.
    These units provide access to data for 16 threads per clock cycle.
  \item[4 SFUs] Special Function Units for execution of transcendental instructions
    like sine, cosine, reciprocal and square root
  \item[2 Warp Schedulers] thread scheduling units that choose 2 packs of 16
    threads for concurrent execution.
  \item[Register file] area of fast memory 128KiB in size\footnote{or $2^{15}$ 32-bit elements}
  \item[Shared memory/L1 cache] 64 KiB of fast memory configurable as
...
These 16 SMs are accompanied by 6 64-bit GDDR5 memory units forming 384-bit
memory interface supporting up to 6GiB of DRAM, 768KiB of L2 cache, GigaThread
engine for distributing blocks\footnote{or work--groups in OpenCL nomenclature}
among SMs and host interface\footnote{PCI-Express bus}.

\subsubsection{Mapping to OpenCL logical hierarchy}

Single Fermi GPU is visible as one OpenCL device. SMs are equivalent of Compute
