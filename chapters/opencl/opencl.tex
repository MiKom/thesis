...
Kernel code must be fed to \texttt{clCreateProgramWithSource()} function in the form of
...
\texttt{clCreateProgramWithBinary()} that takes binary representation of OpenCL
...
by \texttt{clBuildProgram()} function.
...
function. Kernels are scheduled for execution on \emph{command queues} --- special objects that are tied to
particular device and are used for management of tasks on this device.
